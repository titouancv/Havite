...
export function useRecapVote(recapId: string, userId?: string) {
  ...
  const loadVotes = async () => {
    ...
    const { data, error } = await supabase.from('recap_votes').select('*').eq('recap_id', recapId)
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'recap_votes' }, () => {
        loadVotes()
      })
      .subscribe()
  ...
  const vote = async (value: 1 | -1) => {
    ...
    try {
      ...
      if (existing.data && existing.data.vote === value) {
        await supabase.from('recap_votes').delete().eq('recap_id', recapId).eq('user_id', userId)
      } else {
